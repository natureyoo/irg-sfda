...
class student_sfda_RCNN(nn.Module):
    ...
    def extract_features(
        ...
        gt_boxes = [x["instances"].to(self.device) for x in batched_inputs]
        gt_classes = [x.gt_classes for x in gt_boxes]
        ...
        box_features, predictions = self.roi_heads.forward_with_gt_boxes(features, gt_boxes)
        gt_classes = torch.cat(gt_classes)

        if 'instances_synth' in batched_inputs[0]:
            synth_boxes = [x["instances_synth"].to(self.device) for x in batched_inputs]
            synth_classes = [x.gt_classes + 9 for x in synth_boxes]
            synth_box_features, synth_predictions = self.roi_heads.forward_with_gt_boxes(features, synth_boxes)
            box_features = torch.cat([box_features, synth_box_features], dim=0)
            predictions = (torch.cat([predictions[0], synth_predictions[0]]), torch.cat([predictions[1], synth_predictions[1]]))
            gt_classes = torch.cat([gt_classes] + synth_classes)

        return box_features, predictions, gt_classes
